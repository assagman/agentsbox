...
const COMMAND_FILE_PATH = `${COMMAND_DIR}/agentsbox-status.md`;
const COMMAND_CONTENT = `---
description: Check agentsbox plugin status and server health
---
Run agentsbox_status({}) tool and show me the results in a readable format.

Include:
1. MCP Servers table (name, type, tools, status)
2. Tool distribution chart
3. Agentsbox's own tools list (from agentsboxTools field)
4. Health status
...
const EXECUTE_DESC = `Execute a tool discovered via agentsbox_search_bm25 or agentsbox_search_regex.
...
/**
 * Create or update /agentsbox-status slash command
...
export const AgentsboxPlugin: Plugin = async (_ctx: PluginInput) => {
  ...
  // Create /agentsbox-status command file if it doesn't exist
  ensureCommandFile();
  ...
  return {
    tool: {
      ...
      /**
       * Tool Executor
       * Use this to run a tool discovered via agentsbox_search_bm25 or agentsbox_search_regex.
       */
      agentsbox_execute: tool({
    ...
    // Tools are discoverable via agentsbox_search_* once servers connect.
...
export default AgentsboxPlugin;
