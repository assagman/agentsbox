...
export function searchWithRegex(
  ...
  let regex: RegExp;
  try {
    regex = new RegExp(searchPattern, flags); // nosemgrep: javascript.lang.security.audit.detect-non-literal-regexp.detect-non-literal-regexp (user-provided pattern; length-limited + caught)
  } catch (error) {
    return {
      error: {
