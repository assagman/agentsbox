...
2. Create Toolbox config (`~/.config/agentsbox/config.jsonc`):
```jsonc
{
...
- **Custom:** Set `AGENTSBOX_CONFIG` environment variable
...
```
https://unpkg.com/agentsbox@0.8.0/agentsbox.schema.json
```
...
1. Check file exists: `ls ~/.config/agentsbox/config.jsonc`
2. Verify JSON syntax: `cat ~/.config/agentsbox/config.jsonc | bun -e "console.log(JSON.parse(await Bun.stdin.text()))"`
